...
for fixcost in np.arange(200, 501, 50):
        for sd in range(5):
                temp_y, temp_z, temp_obj, temp_time = sprun(9, 4, 40, sd, fixcost)
                temp_y = temp_y.tolist()
                y.append(temp_y)
                sd_list.append(sd)
                open_num.append(sum(temp_y))
                cost1_ = sum(temp_y)*fixcost
                cost1.append(cost1_)
...
for fixcost in np.arange(200, 501, 50):
        for sd in range(5):
                temp_y, temp_z, temp_obj, temp_time = wassrun(9, 4, 40, sd, 0, 1, fixcost)
                temp_y = temp_y.tolist()
                y.append(temp_y)
                sd_list.append(sd)
                open_num.append(sum(temp_y))
...
for fixcost in np.arange(200, 501, 50):
        for sd in range(5):
                _, temp_y, temp_z, temp_obj, temp_time = madrun(9, 4, 40, sd, 0, fixcost)
                temp_y = temp_y.tolist()
                y.append(temp_y)
                sd_list.append(sd)
                open_num.append(sum(temp_y))
...
with pd.ExcelWriter('sensitivity/impact_fixcost.xlsx', engine='openpyxl') as writer:
